...
function App() {
  ...
  const { includeNames, showNextLetters, setIncludeNames, setShowNextLetters } = useSettings();
  const [settingsOpen, setSettingsOpen] = useState(false);

  const [bearVisible, setBearVisible] = useState(false);
  const [duckVisible, setDuckVisible] = useState(false);

  const { list: fishList, spawn: spawnFish, remove: removeFish } =
    useSpawner("fish", randomFishTop, randomFishDuration);
  const { list: horseList, spawn: spawnHorse, remove: removeHorse } = useSpawner(
    "horse",
    () => {
  ...
  useEffect(() => wordState.cleanupWordDetection, [wordState.cleanupWordDetection]);
  ...
  return (
    <div className="toddler-app">
      ...
      <SettingsModal
        ...
        onShowNextLettersChange={setShowNextLetters}
      />
      ...
      {isMobile && wordState.availableLetters.length > 0 ? (
        ...
        <div className="display-area">
          {showDesktopBuilder ? (
            <DesktopWordBuilder
              desktopTypedSequenceDisplay={wordState.desktopTypedSequenceDisplay}
